...
class Shop extends BaseController
{
  private $url = "https://api.rajaongkir.com/starter/";
	private $apiKey = "6397b5d9ebb3c076511d72364b889573";
  ...
  { 
    ...
    public function product()
	{
        ...
        $provinsi = $this->bantuan->rajaongkir($this->url."province",$this->apiKey);
    ...
    public function getCity()
    {
        if ($this->request->isAJAX()){
            ...
            $data = $this->bantuan->rajaongkir($this->url."city?province=".$id_province,$this->apiKey);
    ...
    public function getDiskon(){
      if($this->request->isAjax()){
        $diskon_request=$this->request->getGet('diskon'); //mengambil data request dari frontend (product)
        $diskon_data = $this->diskon->select('diskon.*')->where('kode_voucher',$diskon_request)->findAll(); //pengecekan di bd ada tidak
        if(!$diskon_data){ //mengecek apakah kode jika tidak ada lalu dikirimkan ke view > shop > product
          return $this->response->setJSON('Diskon tidak ditemukan');
        }
        return $this->response->setJSON($diskon_data); //jika ada data dikirim ke product
      }
    }
