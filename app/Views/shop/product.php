                            ...
                            <div class="product-details">
                                ...
                                <div class="pd-desc"> 
                                    ...
                                    <?= form_open('buy') ?>
                                        ...
                                        <!-- UNTUK MEMBERITAHU APAKAH DISKON ADA/TIDAK DI FRONTEND -->
...
<script>
    ...
    $("#diskon").on('change', function(){
			var diskon_val = $(this).val(); //AMBIL dari form_open
      console.log(diskon_val) 
			$.ajax({ //API
				...
				dataType : 'json',
				success : function(data){ //data diambil dari shop jika ada data
					console.log(data[0].tanggal_berakhir); //pengecekan aktif tidaknya diskon
          const api_date = data[0].tanggal_berakhir
          const dateNow = `${new Date().getFullYear()}-${new Date().getMonth()+1}-${new Date().getDate()}`
          if(api_date > dateNow){ //jika data pada diskon lebih besar dari tanggal sekarang maka
